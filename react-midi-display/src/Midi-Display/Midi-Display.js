        ...
        <div style={{ overflowY: 'scroll', overflowX: 'scroll', whiteSpace: 'nowrap', display: 'flex', flexDirection: 'row', alignItems: 'flex-start'}}>
            ...
            { (() => {
                ...
                for (let i = numNotes[0]; i <= numNotes[1]; i++)
                {
                    ...
                    if (i % 12 == 1 || i % 12 == 3 || i % 12 == 6 || i % 12 == 8 || i % 12 == 10)
                    {
                        divArray.push(<div key={i} style={{position: 'absolute', left: (lastWhiteKeyPosition + (keyWidth + 1) - (keyWidth / 4))+ 'px', display: 'inline-block', width: (keyWidth / 2)+'px', height: (keyHeight * (5/8))+'px', backgroundColor: 'black', zIndex: 2}}></div>);
                    }else{
                        ...
                        divArray.push(<div key={i} style={{position: 'absolute', left: lastWhiteKeyPosition + 'px', boxShadow: "inset 0 0 0 1px black", display: 'inline-block', width: keyWidth+'px', height: keyHeight+'px', backgroundColor: 'white'}}></div>);
                    }
                }
